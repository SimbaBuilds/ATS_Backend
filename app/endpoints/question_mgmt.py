...
from app.models import QuestionBankQuestion, PracticeTestQuestion  # Your SQLAlchmey models
...
async def get_question(question_id: int, db: Session = Depends(get_db)):
    question = db.query(QuestionBankQuestion).filter(QuestionBankQuestion.id == question_id).first()
    if not question:
...
async def get_practice_test(test_id: int, db: Session = Depends(get_db)):
    practice_test = db.query(PracticeTestQuestion).filter(PracticeTestQuestion.id == test_id).first()
...
async def get_practice_test_question(practice_test: str, question_number: int, db: Session = Depends(get_db)):
    practice_test_question = db.query(PracticeTestQuestion).filter(PracticeTestQuestion.practice_test == practice_test and PracticeTestQuestion.question_number == question_number).first()
